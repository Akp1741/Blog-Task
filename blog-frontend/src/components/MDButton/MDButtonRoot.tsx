...
export default styled(Button)<CustomButtonProps>(({ theme, ownerState }) => {
  ...
  const { color, variant, size, circular, iconOnly, darkMode }:any = ownerState;

  const { white, text, transparent, gradients, grey } :any= palette;
  const { boxShadow, linearGradient, pxToRem, rgba } :any= functions;
  const { borderRadius }:any = borders;
  const { colored } :any= boxShadows;
